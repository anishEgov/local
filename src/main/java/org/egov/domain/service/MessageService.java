...
public class MessageService {
	...
	public List<Message> getFilteredMessages(MessageSearchCriteria searchCriteria) {
		var messages = getMessages(searchCriteria);
		var tenant = searchCriteria.getTenantId();
		var tenants = tenant.getTenantHierarchy();
	...
	private List<Message> filterForGivenTenantId(String tenantId, List<Message> messages, MessageSearchCriteria searchCriteria) {
		if (searchCriteria.isModuleAbsent() && !CollectionUtils.isEmpty(searchCriteria.getCodes())) {
			var codes = searchCriteria.getCodes();
			...
			var modules = Arrays.asList(searchCriteria.getModule().split("[,]"));
	...
	public void delete(List<MessageIdentity> messageIdentities) {
		var tenantToMessageIdentitiesMap = messageIdentities.stream()
	...
	private void bustCacheEntriesForMessages(Tenant tenant, List<Message> messages) {
		var messageIdentities = messages.stream()
	...
	private List<Message> getMessages(MessageSearchCriteria searchCriteria) {
		var cachedMessages = messageCacheRepository.getComputedMessages(searchCriteria.getLocale(),
			searchCriteria.getTenantId());
		if (cachedMessages != null) {
			return cachedMessages;
		}
		var computedMessages = computeMessageList(searchCriteria.getLocale(),
			searchCriteria.getTenantId());
	...
	private void deleteMessagesForGivenTenant(Map<Tenant, List<MessageIdentity>> tenantToMessageIdentitiesMap,
			Tenant tenant) {
		var messageIdentitiesForGivenTenant = tenantToMessageIdentitiesMap.get(tenant);
		var localeToMessageIdentitiesMap = messageIdentitiesForGivenTenant.stream()
	...
	private void deleteMessagesForGivenLocale(Tenant tenant,
		...
		var messageIdentitiesForGivenLocale = localeToMessageIdentitiesMap.get(locale);
		var moduleToMessageIdentitiesMap = messageIdentitiesForGivenLocale.stream()
	...
	private void deleteMessagesForGivenModule(Tenant tenant, String locale,
		...
		var messageIdentitiesForGivenModule = moduleToMessageIdentitiesMap.get(module);
		var codes = getCodesForGivenMessage(messageIdentitiesForGivenModule);
	...
	private List<Message> computeMessageList(String locale, Tenant tenant) {
		var messagesForGivenLocale = getMessagesForGivenLocale(locale, tenant);
		var defaultMessages = getDefaultMessagesForMissingCodes(messagesForGivenLocale);
	...
	private List<Message> getDefaultMessagesForMissingCodes(Collection<Message> messagesForGivenLocale) {
		var messagesInEnglishForDefaultTenant = fetchMessageFromRepository(ENGLISH_INDIA,
			new Tenant(Tenant.DEFAULT_TENANT));

		var messageCodesInGivenLanguage = new HashSet<String>();
	...
	private Collection<Message> getMessagesForGivenLocale(String locale, Tenant tenant) {
		var codeToMessageMap = new HashMap<String, Message>();
		var messages = tenant.getTenantHierarchy().stream()
			.map(tenantItem -> fetchMessageFromRepository(locale, tenantItem))
			.flatMap(List::stream)
			.toList();

		messages.forEach(message -> {
			var key = message.getModule() + message.getCode();
			var matchingMessage = codeToMessageMap.get(key);
			if (matchingMessage == null) {
				codeToMessageMap.put(key, message);
			} else if (message.isMoreSpecificComparedTo(matchingMessage)) {
				codeToMessageMap.put(key, message);
			}
		});
	...
	private List<Message> getEnglishMessagesForCodesNotPresentInLocalLanguage(Set<String> messageCodesForGivenLocale,
		...
		return messagesInEnglish.stream()
			.filter(message -> !messageCodesForGivenLocale.contains(message.getModule() + message.getCode()))
			.toList();
	}

	private List<Message> fetchMessageFromRepository(String locale, Tenant tenant) {
		var cachedMessages = messageCacheRepository.getMessages(locale, tenant);
		if (cachedMessages != null) {
			return cachedMessages;
		}
		var messages = messageRepository.findByTenantIdAndLocale(tenant, locale);
